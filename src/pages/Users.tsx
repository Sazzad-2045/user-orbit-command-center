...
export default function Users() {
	...
	return (
		<div className="p-6">
			...
			{/* Edit User / Change Plan Dialog */}
			<Dialog open={!!editingUser} onOpenChange={(open) => !open && setEditingUser(null)}>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>
							{editingUser?.changePlan ? "Change User Plan" : "Edit User Information"}
						</DialogTitle>
					</DialogHeader>
					{editingUser && (
						<div className="space-y-4">
							{!editingUser.changePlan ? (
								<>
									<div>
					...
					<DialogFooter>
						...
						<Button onClick={() => {
							if (editingUser?.changePlan) {
								const updated = { ...editingUser }
								delete updated.changePlan
								handleEditUser(updated)
							} else {
								handleEditUser(editingUser)
							}
						}}>
							{editingUser?.changePlan ? "Update Plan" : "Save Changes"}
						</Button>
					</DialogFooter>
				</DialogContent>
			</Dialog>
